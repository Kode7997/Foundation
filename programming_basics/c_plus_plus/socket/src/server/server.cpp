...
std::string process_request(const std::string& xml_input) {
    ...
    try {
        ...
        int op1 = pt.get<int>("request.data.num1");
        int op2 = pt.get<int>("request.data.num2");
        ...
        std::cerr << "XML Parsing Exception: " << e.what() << "\n";
        return std::string("<response><error>Parsing error: ") + e.what() + "</error></response>";
    }
}
